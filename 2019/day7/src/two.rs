...
fn main() -> io::Result<()> {
    let code = include_str!("../input")
        ...
        .map(Result::unwrap)
        .collect::<Vec<_>>();
    let mut phase_seq = [8, 8, 7, 6, 5];
    println!(
        "{}",
        std::iter::from_fn(|| {
            if phase_seq.iter().all(|i| *i == 9) {
                return None;
            }
        ...
        .map(|seq| {
            ...
            let mut threads = Vec::with_capacity(seq.len());
            let mut work_tokens = Vec::with_capacity(seq.len());
            for i in seq.iter().rev() {
                let (pp0, mut pp1) = pipe();
                write!(pp1, "{}", i).expect("Error writing initial value");
                let code_clone = code.clone();
                let token = Arc::new(());
                work_tokens.push(Arc::downgrade(&token));
                threads.push(thread::spawn(move || {
                    let _ = token;
                    run(code_clone, pp0, old_p)
                }));
                old_p = pp1;
            }
            ...
            let mut last = 0_i64;
            loop {
                last = match output.0.recv() {
                    Ok(v) => v,
                    Err(_) => break
                };
                if let Err(_) = old_p.0.send(last) {
                    break;
                }
            }
